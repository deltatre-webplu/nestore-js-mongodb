  ...
  describe("When connected", function () {
    ...
    describe("Giving a bucket", function () {
      ...
      describe("Read commits", function () {
          ...
          return new Promise<void>((resolve, reject) => {
            projection
              .on("data", (doc: CommitData) => {
                docs.push(doc);
                if (doc._id === 3) {
                  // when last event is read close the projection
                  projection.close();
                }
              })
              .on("error", (err: Error) => {
        ...
        it("should be possible to create a projection stream and wait for new events", function () {
          ...
          return new Promise<void>((resolve, reject) => {
            projection
              ...
              .on("wait", (data: { filters: CommitsFilters }) => {
                // when first stream is completed add a new commit
                waitCalls++;
        ...
        it("should be possible to create a projection and close it multiple times", async function () {
          const projection = bucket.projectionStream({}, { waitInterval: 100 });

          const projectionChecks = new Promise<void>((resolve, reject) => {
            projection
              .on("error", (err: Error) => {
                projection.close();
                reject(err);
      ...
      describe("Read and write commits", function () {
        it("Real world usage", async function () {
          ...
          for (let i = 0; i < 10; i++) {
            await bucket.write(streamId, i, [{ value: i }], {
              dispatched: true,
            });
          }
